...
// --- PERUBAHAN DI SINI ---
// Menambahkan systemInstruction untuk memberikan persona pada AI
const model = genAI.getGenerativeModel({ 
    model: "gemini-2.5-pro",
    systemInstruction: `Mulai sekarang kamu berperan sebagai Gemini Code Assistant".

Aturan utama:
1. Selalu jawab dengan kode **lengkap** yang bisa langsung dicoba.
2. Jangan kasih penjelasan panjang atau teori.
3. Jangan tambahkan komentar apapun dalam kode.
4. Selalu kirim codingan tanpa tanda ;
5. Kode harus rapi, flatten (mojok ke kiri semua).
6. Jika diminta solusi, langsung tulis implementasi, bukan penjelasan.
7. Jangan membantah atau ngeyel jika ada koreksi.
8. Bahasa komunikasi hanya untuk instruksi, bukan output.`
})
// -------------------------
...
export async function handleAiChat2(ctx) {
  ...
  try {
    ...
    userDb.codeassistantHistory = newHistory.slice(-HISTORY_LIMIT); await global.db.write()

  } catch (error) {
    ctx.handleError(error, 'codeassistant')
...
export default {
    ...
    execute: async (ctx) => {
      ...
      const userDb = global.db.data.users[userId]
      if (action === 'on') { userDb.codeassistantActive = true; await ctx.reply('✅ Mode Code Assistant telah diaktifkan.') }
      else if (action === 'off') { userDb.codeassistantActive = false; await ctx.reply('❌ Mode Code Assistant telah dinonaktifkan.') }
      else if (action === 'clear') { userDb.codeassistantHistory = []; await ctx.reply('🧹 Riwayat percakapan AI telah dibersihkan.') }
      else { const status = userDb.codeassistantActive ? '🟢 Aktif' : '🔴 Nonaktif'; await ctx.replyWithMarkdown(`*Mode Code Assistant saat ini:* ${status}\n\nGunakan:\n• \`/codeassistant on\`\n• \`/codeassistant off\`\n• \`/codeassistant clear\``) }
