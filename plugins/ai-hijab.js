...
export default {
  ...
  execute: async (ctx) => {
    ...
    if (ctx.message.photo) {
      ...
      const commandRegex = /^\/(hijabkan|tohijab)\s*/
    ...
    if (!targetPhoto) {
      return ctx.reply('Mohon kirimkan foto langsung dengan caption /hijabkan atau reply foto dengan perintah ini.', { reply_to_message_id: ctx.message.message_id })
    }
    ...
    try {
      ...
      ctx.handleError(error, 'hijabkan')
    }
  }
}
